...
import { BidRequestBuilder } from '../core/bid-request-builder';
import { AuctionEngine } from '../core/auction-engine';
...
export async function registerAdRequestRoute(
  fastify: FastifyInstance,
  dspAdapters: DSPAdapter[],
) {
  ...
  const dspManager = new DSPManager(dspAdapters);

  const auctionEngine = new AuctionEngine({
    async fireWinNotice(url: string) {
      try {
  ...
  fastify.get('/ad-request', async (request, reply) => {
    ...
    let slots: Array<{ id: string; sizes: string[]; floor?: number }> = [];
    try {
    ...
    const responses: Array<{ dspId: string; response: BidResponse | null }> = fanOutResults;
    ...
    const slotsResponse = auctionResult.impressionResults.map((imp) => {
      ...
      return {
        ...
        impressionBeacon: `/beacon?type=imp&auc=${encodeURIComponent(
          auctionResult.auctionId,
        )}&imp=${encodeURIComponent(imp.impId)}`,
        viewabilityBeacon: `/beacon?type=view&auc=${encodeURIComponent(
          auctionResult.auctionId,
        )}&imp=${encodeURIComponent(imp.impId)}`,
        clickBeacon: `/beacon?type=click&auc=${encodeURIComponent(
          auctionResult.auctionId,
        )}&imp=${encodeURIComponent(imp.impId)}`,
      };
    });
