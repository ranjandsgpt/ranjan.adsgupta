...
import { getRecentAuctions, getStatusMetrics } from '../services/metrics-store';

const API_KEY = process.env.EXCHANGE_API_KEY || '';

function requireApiKey(fastify: FastifyInstance) {
...
export async function registerApiRoutes(fastify: FastifyInstance) {
  ...
  api.register(async (instance) => {
    ...
    }>('/api/auctions', async (request, reply) => {
      const limit = Math.min(
        100,
        Math.max(1, parseInt(request.query.limit || '50', 10) || 50),
      );
      const auctions = getRecentAuctions(limit);
