...
const recentAuctions: AuctionSummary[] = [];
...
export function recordAuction(summary: Omit<AuctionSummary, 'timestamp'>) {
  const ts = Date.now();
  trimHour();
  auctionsLastHour += 1;
  totalAuctionCalls += 1;
...
export function getRecentAuctions(limit: number = 50): AuctionSummary[] {
...
export function getStatusMetrics() {
  trimHour();
  const fillRate =
    totalRequestCount > 0 ? (totalFillCount / totalRequestCount) * 100 : 0;
  const avgLatencyMs =
    totalAuctionCalls > 0 ? Math.round(totalLatencyMs / totalAuctionCalls) : 0;
  return {
